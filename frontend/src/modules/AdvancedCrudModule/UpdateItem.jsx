...
import { selectUpdatedItem } from '@/redux/adavancedCrud/selectors';
...
function SaveForm({ form, translate }) {
  ...
  return (
    <Button onClick={handelClick} type="primary" icon={<PlusOutlined />} className="customButton">
      {translate('update')}
    </Button>
  );
}
...
export default function UpdateItem({ config, UpdateForm }) {
  ...
  const [currentAdavancedCrud, setCurrentAdavancedCrud] = useState(current ?? resetAdavancedCrud);
  ...
  const onSubmit = (fieldsValue) => {
    ...
    dispatch(adavancedCrud.update({ entity, id, jsonData: dataToUpdate }));
  };
  useEffect(() => {
    if (isSuccess) {
      form.resetFields();
      setSubTotal(0);
      dispatch(adavancedCrud.resetAction({ actionType: 'update' }));
  ...
  useEffect(() => {
    if (current) {
      setCurrentAdavancedCrud(current);
